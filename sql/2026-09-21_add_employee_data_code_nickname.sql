/*
    2026-09-21  เพิ่มช่อง "รหัสพนักงาน" (is_employee_code) และ "ชื่อเล่น" (is_nickname)
                ในแบบฟอร์ม "ขอข้อมูลพนักงาน" (FormEmployeeData) — เป็นช่องเดี่ยว (ไม่มี TH/EN)
...
IF NOT EXISTS (
    ...
    WHERE object_id = OBJECT_ID('dbo.FormEmployeeData') AND name = 'is_employee_code'
)
BEGIN
    ALTER TABLE dbo.FormEmployeeData ADD
        is_employee_code int NULL,
        is_nickname int NULL
END
GO
...
begin
	...
	, isnull(is_employee_code, 0) as is_employee_code
	, isnull(is_nickname, 0) as is_nickname
